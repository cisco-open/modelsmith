...
if __name__ == '__main__':
    ...
    parser.add_argument('--awq', default=True, help='weight_quant for input in activation reconstruction')
    parser.add_argument('--aaq', default=True, help='act_quant for input in activation reconstruction')
    ...
    if unknown_args:
        logger.log(f"Warning: Unrecognized arguments {' '.join(unknown_args)}. These will be ignored.", flush=True)
    ...
    print('Full quantization (W{}A{}) accuracy: {}'.format(args.n_bits_w, args.n_bits_a, test(0, device, qnn, test_loader, criterion, best_acc, checkpoint_dir, logger)))
