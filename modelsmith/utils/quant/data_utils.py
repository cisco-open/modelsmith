...
def save_inp_oup_data(model: QuantModel, layer: Union[QuantModule, BaseQuantBlock], cali_data: torch.Tensor,
    ...
    get_inp_out = GetLayerInpOut(model, layer, device=device, wq=wq, aq=aq, input_prob=input_prob)
    cached_batches = []
...
class GetLayerInpOut:
    def __init__(self, model: QuantModel, layer: Union[QuantModule, BaseQuantBlock],
                 device: torch.device, wq: bool = False, aq: bool = False, input_prob: bool = False):
        self.model = model
        self.layer = layer
        self.device = device
        self.wq = wq
