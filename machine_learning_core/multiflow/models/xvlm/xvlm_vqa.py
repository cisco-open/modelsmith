...
class XVLM(XVLMBase):
    ...
    def load_pretrained(self, ckpt_rpath, config, is_eval=False, verbose=True):
        ...
        if verbose:
            print('='*50, "WEIGHTS LOADED", '='*50)
            print('Loaded checkpoint from', ckpt_rpath)
            print(f"MISSING KEYS = {[p for p in msg.missing_keys if 'pruning_mask' not in p]}")
            print(f"UNEXPECTED KEYS = {msg.unexpected_keys}")


    def load_mask(self, mask_path, verbose=True):
        state_dict = torch.load(mask_path, map_location="cpu")
        print("Loading pretrained pruning masks.")
        
        # correctly replace all the mask keys of the text encoder, such that
        # they can be inherited by all the linear layers in the decoder
        for key in list(state_dict.keys()):
            if 'text_encoder.' in key:
                
                # replace the 'bert' string --> 'text_encoder.bert.encoder...mask' becomes 'text_encoder.encoder...mask'
                if 'bert.' in key:
                    encoder_key = key.replace('bert.', '')
                    state_dict[encoder_key] = state_dict[key]
                
                if 'layer.' in key:
                    nested_modules = list(key.split('.'))
                    layer_num = int(nested_modules[4])

                    # if we are looking at a lower layer than 6, we don't need to change the key
                    # as we only inherit this mask for the encoder 
                    if layer_num < self.num_text_layers:
                        del state_dict[key]
                        continue
                    ...
                    # if we are above the 6th text encoder layer, duplicate the weights
                    # this else is the only statement where keys are added to the decoder
                    else:
                        decoder_layer_num = (layer_num - self.num_text_layers)
                        nested_modules[4] = str(decoder_layer_num)
                        decoder_key = '.'.join(nested_modules)
                        decoder_key = decoder_key.replace('text_encoder', 'text_decoder')
                        state_dict[decoder_key] = state_dict[key]
                        del state_dict[key]
        ...
        msg = self.load_state_dict(state_dict, strict=False)
        if verbose:
            print('='*50, "MASKS LOADED", '='*50)
            print('Loaded mask from:', mask_path)
            # since we are loading only the mask, it is trivial that weight params will be missing keys;
            # let's filter the message to the pruning masks we are missing
            print(f"MISSING KEYS = {[p for p in msg.missing_keys if 'pruning_mask' in p and '.bias' not in p]}")
            print(f"UNEXPECTED KEYS {msg.unexpected_keys}")


    def load_from_pruned_pretrained(self, weights_path, mask_path, config, verbose=True):
        r"""
        This methods load pretraining weights from an XVLMPretrain model, as well as 
        the pruning mask of the backbone. It is expected that the weights and the mask
        hold the same keys, and that the mask is a subset of the weights. The naming
        convention is that each key matches with the keys in the .state_dict() method 
        of XVLMPretrain instances.
        """

        # load the weights using the native method
        self.load_pretrained(weights_path, config, is_eval=False, verbose=verbose)

        # load the mask using the custom method
        # NOTE: for some reason, parameters in the decoder are called 'text_decoder.bert.layer ...' instead of
        #       'text_decoder.layer ...' as in the encoder. This is why we need to insert 'bert' in the decoder
        self.load_mask(mask_path, verbose=verbose)
