...
class CLIPEncoderLayer(nn.Module):
    def __init__(self, hidden_size, hidden_act, num_attention_heads, attention_dropout, intermediate_size, drop_ln=False):
        super().__init__()
...
class CLIPEncoder(nn.Module):
    ...
    def __init__(self, hidden_size, hidden_act, num_attention_heads, attention_dropout, intermediate_size, num_hidden_layers, local_attn_depth, save_forward_data=True, drop_ln=False):
        super().__init__()
        self.depth = num_hidden_layers
        self.local_attn_depth = local_attn_depth
        self.layers = nn.ModuleList([CLIPEncoderLayer(hidden_size, hidden_act, num_attention_heads, attention_dropout, intermediate_size, drop_ln) for _ in range(num_hidden_layers)])
        self.save_forward_data = save_forward_data

        # initialize buffers for hidden states and attention maps (useful in distillation settings)
        self.hidden_states, self.attn_weights = [None for _ in range(len(self.layers))], [None for _ in range(len(self.layers))]
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            if (self.local_attn_depth > 0) and (idx >= self.depth - self.local_attn_depth):
                ...
                hidden_states, attn_weights = encoder_layer(hidden_states, attention_mask=image_atts_blk)
            else:
                hidden_states, attn_weights = encoder_layer(hidden_states, attention_mask=None)
            
            if self.save_forward_data:
...
class CLIPVisionTransformer(nn.Module):
    def __init__(self, image_size, patch_size, hidden_size, hidden_act, num_attention_heads, attention_dropout, intermediate_size, num_hidden_layers, 
                 local_attn_depth=0, save_forward_data=True, drop_ln=False):
        super().__init__()
        ...
        self.encoder = CLIPEncoder(hidden_size, hidden_act, num_attention_heads, attention_dropout, intermediate_size,
                                   num_hidden_layers, local_attn_depth=local_attn_depth, save_forward_data=save_forward_data, drop_ln=drop_ln)
