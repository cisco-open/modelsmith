...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.drop_ln = getattr(config, "drop_ln", False)
    ...
    def forward(
        ...
        # print("token type ids:", token_type_ids.size() if isinstance(token_type_ids, torch.Tensor) else token_type_ids)
        # print("position_ids:", position_ids.size() if isinstance(position_ids, torch.Tensor) else position_ids)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention):
        ...
        self.save_attention = config.save_attention
        self.save_hidden = config.save_hidden
            
    def save_attn_gradients(self, attn_gradients):
        if hasattr(self, "attn_gradients"):
    ...
    def save_attention_map(self, attention_map):
        if hasattr(self, "attention_map"):
            del self.attention_map
        self.attention_map = attention_map

    def save_attention_scores(self, attention_scores):
        if hasattr(self, "attention_scores"):
            del self.attention_scores
        self.attention_scores = attention_scores

    def save_attn_scores_gradients(self, attn_scores_gradients):
        if hasattr(self, "attn_scores_gradients"):
            del self.attn_scores_gradients
        self.attn_scores_gradients = attn_scores_gradients

    def get_attn_scores_gradients(self):
        return self.attn_scores_gradients
    ...
    def save_hidden_states(self, hidden_states):
    ...
    def save_hidden_gradients(self, hidden_gradients):
        if hasattr(self, "hidden_gradients"):
            del self.hidden_gradients
        self.hidden_gradients = hidden_gradients
        
    def get_hidden_gradients(self):
        return self.hidden_gradients
    ...
    def forward(
        ...
        if self.save_attention:
            ...
            self.save_attention_scores(attention_scores)
            # NOTE: as of now, gradients w.r.t. attention scores are not needed
            if torch.is_grad_enabled():
                attention_probs.register_hook(self.save_attn_gradients)
                attention_scores.register_hook(self.save_attn_scores_gradients)
        ...
        if self.save_hidden:
            self.save_hidden_states(context_layer)
            if torch.is_grad_enabled():
                context_layer.register_hook(self.save_hidden_gradients)
...
class BertLayer(nn.Module):
    ...
    def get_attn_weights(self):
        if hasattr(self, "crossattention"):
            return self.crossattention.self.get_attention_map()
        else:
            return self.attention.self.get_attention_map()
        
    def get_hidden_states(self):
        if hasattr(self, "crossattention"):
            return self.crossattention.self.get_hidden_states()
        else:
            return self.attention.self.get_hidden_states()
    ...
    def forward(
        ...
        # decoder uni-directional self-attention cached key/values  is at positions 1,2
...
class BertEncoder(nn.Module):
    ...
    def get_hidden_states(self):
        return [layer.get_hidden_states() for layer in self.layer]
    
    def get_attn_weights(self):
        text_attn_weights = [self.layer[i].get_attn_weights() for i in range(self.config.fusion_layer)]
        fusion_attn_weights = [self.layer[i].get_attn_weights() for i in range(self.config.fusion_layer, len(self.layer))]
        return (text_attn_weights, fusion_attn_weights)
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        # this variable does not seem to be used and causes issues
        # when using distributed training as it doesn't receive gradients
        
        # self.bias = self.decoder.bias.clone()
...
class BertModel(BertPreTrainedModel):
    ...
    def get_attn_weights(self):
        return self.encoder.get_attn_weights()
    ...
    def forward(
        ...
        # print("bert model forward token type ids:", token_type_ids.size() if token_type_ids is not None else token_type_ids)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if custom_temp is None:
                ...
                masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size) / custom_temp, labels.view(-1))
