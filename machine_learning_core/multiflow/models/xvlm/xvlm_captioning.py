...
class XVLM(XVLMBase):  
    def __init__(self, config):
        ...
        assert config['text_encoder'] in ('data/bert-base-uncased', 'bert-base-uncased')
    ...
    def load_from_pruned_pretrained(self, weights_path, masks_path, config, load_capt_pretrain=False, verbose=True):
        """
            Load from a .state_dict() call of an instance of XVLM for Captioning Pretraining.
            The provided state dict must also contain the pbuffers for the pruning masks.
        """

        if masks_path is not None and load_capt_pretrain:
            raise ValueError("Ambiguous setting. Please set either masks_path=None of load_capt_pretrain=False")
        
        state_dict = load_pretrained(weights_path, config, load_text=False)
        if not load_capt_pretrain:
            for key in list(state_dict.keys()):
                assert isinstance(key, str)
                if key.startswith('text_encoder.'):
        ...
        msg = self.load_state_dict(state_dict, strict=False)
        if verbose:
            print('load checkpoint from %s' % weights_path)
            print("missing_keys: ", [p for p in msg.missing_keys if 'vision_encoder' not in p])
            print("unexpected_keys: ", msg.unexpected_keys)
        ...
        if masks_path is not None:
            ...
            for key in list(masks_dict.keys()):
                ...
                if key.startswith('text_encoder.'):
                    decoder_key = key.replace('text_encoder.', 'text_decoder.')
                    masks_dict[decoder_key] = masks_dict[key]
                    del masks_dict[key]
            msg = self.load_state_dict(masks_dict, strict=False)
            if verbose:
                print('load masks from %s' % masks_path)
    ...
    def forward(self, image, caption, already_tokenized=False):
        ...
        if not already_tokenized:
            text = self.tokenizer(caption, padding='longest', truncation=True, max_length=self.max_tokens, return_tensors="pt").to(image.device)
        else:
            text = caption
    ...
    def generate(self, image, sample=False, num_beams=1, max_length=30, min_length=10, top_p=0.9,
        ...
        # NOTE: the src code at <repo_link> used transformers=4.12.5; with that version, manually repeating
        # the encoder hidden states is needed for beam search. In this repo we upgraded to transformers=4.31.0, where 
        # such manual repetition will cause errors. Therefore, we comment out the following lines.
        ...
        # NOTE: since it is the default setting in 'configs/captioning.yaml', this code only supports 
        # using beam search for text generation. Sampling and greedy generation have NOT been tested.
